...
async function createMidtransPayment(order) {
  ...
  const items = [];
  if (order.nastar_qty    > 0) items.push({ id:'NASTAR',    name:'Premium Nastar',    price:95000, quantity:order.nastar_qty    });
  if (order.kastangel_qty > 0) items.push({ id:'KASTANGEL', name:'Premium Kastangel', price:95000, quantity:order.kastangel_qty });
