...
<div style="max-width:560px;margin:40px auto;font-family:Georgia,serif;background:#FFFDF8;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08)">
  ...
  <div style="padding:36px">
    ...
    ${order.nastar_qty    > 0 ? `<p style="color:#3D1F0D">🍡 Premium Nastar ×${order.nastar_qty} — <strong>${fmt(order.nastar_qty * 80000)}</strong></p>` : ''}
    ${order.kastangel_qty > 0 ? `<p style="color:#3D1F0D">🧀 Premium Kastangel ×${order.kastangel_qty} — <strong>${fmt(order.kastangel_qty * 115000)}</strong></p>` : ''}
